...
resizing_model = tf.keras.Sequential([
    layers.Resizing(200, 200, interpolation='bilinear', crop_to_aspect_ratio=True)
])

prediction_model = create_model()

# Use the provided weights file
weights_path = 'GUI/training/cp.ckpt.weights.h5'
...
root = tkinter.Tk()
root.title('Proggapon ESL Translator')

title = tkinter.Label(root,
                      text='Progaapon ESL Translator',
